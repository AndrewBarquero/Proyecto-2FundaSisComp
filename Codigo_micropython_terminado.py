...
def recibir_mensaje():
    global conn, en_mantenimiento

    while True and conn != None:
        print("Hola")
        data = conn.recv(1024)
        if not data:
            continue
...
led_verde = Pin(12,Pin.OUT) #Led verde que se encendera cuando se pueda comprar algun producto
led_rojo =  Pin(11,Pin.OUT) #Led rojo que se encendera cuando no haya de un producto y no se pueda comprar

buzzer = Pin(0, Pin.OUT) #Buzzer que indica cuando se esta en modo mantenimiento
...
#Se hace un servo para producto, para que cada producto tenga su puerta
servoX =  PWM(Pin(28)) #Por la naturaleza del PWM no es necesario el Pin.OUT
servoY =  PWM(Pin(26))
servoZ =  PWM(Pin(22))
servoX.freq(50) #el 50 son los hz en los que opera el servo motor
servoY.freq(50)
servoZ.freq(50)
...
def mover_servoX():
    servoX.duty_ns(v_0_grados) 
    time.sleep(5) #Aqui espera 5 segundos para que el usuario tenga el tiempo suficiente para obtener el producto
    servoX.duty_ns(v_180_grados) 
    time.sleep(1)
def mover_servoY():
    servoY.duty_ns(v_0_grados) 
    time.sleep(5) 
    servoY.duty_ns(v_180_grados) 
    time.sleep(1)
def mover_servoZ():
    servoZ.duty_ns(v_0_grados) 
    time.sleep(5) 
    servoZ.duty_ns(v_180_grados) 
    time.sleep(1)
...
def interaccion_producto_maquina(num):
    global cantidad_de_x_producto, cantidad_de_y_producto, cantidad_de_z_producto, conn #Para cambiar las variables de productos se llaman con global
